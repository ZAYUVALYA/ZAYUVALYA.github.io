...
while true; do
  ...
  read input
  if [[ "$input" == "exit" ]]; then
    echo -e "${YELLOW}Exiting... See you next time!${NC}"
    break
  fi
  ...
  echo -e "${GRAY}[${timestamp}] Thinking...${NC}"
  start_time=$(date +%s.%N)
